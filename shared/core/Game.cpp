...
Game::Game(std::vector<PiecePtr> pcs, Board board)
    ...
    // Setup key mappings for Player 2 (Arrow keys + Enter)
    keymap_player2_ = {
        {2490368, "left"},   // Left arrow
        {2555904, "down"},   // Down arrow
        {2424832, "up"},     // Up arrow
...
Game::Game(Game&& other) noexcept
    : pieces(std::move(other.pieces)),
      ...
      background_img_(std::move(other.background_img_)),
      piece_by_id(std::move(other.piece_by_id)),
      pos(std::move(other.pos)),
      user_input_queue(std::move(other.user_input_queue)),
      string_input_queue(std::move(other.string_input_queue)),
      running_(other.running_.load()),
      selected_piece_player1_(std::move(other.selected_piece_player1_)),
      cursor_pos_player1_(other.cursor_pos_player1_),
      selected_piece_pos_player1_(other.selected_piece_pos_player1_),
      is_selecting_target_player1_(other.is_selecting_target_player1_),
      selected_piece_player2_(std::move(other.selected_piece_player2_)),
      cursor_pos_player2_(other.cursor_pos_player2_),
      selected_piece_pos_player2_(other.selected_piece_pos_player2_),
      is_selecting_target_player2_(other.is_selecting_target_player2_),
      keymap_player1_(std::move(other.keymap_player1_)),
      keymap_player2_(std::move(other.keymap_player2_)),
      start_tp(other.start_tp),
      ...
      messageDuration_(other.messageDuration_),
      showMessage_(other.showMessage_),
      eventManager_(std::move(other.eventManager_)),
      whiteMovesTracker_(std::move(other.whiteMovesTracker_)),
      blackMovesTracker_(std::move(other.blackMovesTracker_)),
      whiteScoreTracker_(std::move(other.whiteScoreTracker_)),
      blackScoreTracker_(std::move(other.blackScoreTracker_)),
      voiceAnnouncer_(std::move(other.voiceAnnouncer_)),
      soundManager_(std::move(other.soundManager_)),
      network_interface_(other.network_interface_),
      my_player_id_(other.my_player_id_) {
    // Reset other object
    other.network_interface_ = nullptr;
    other.my_player_id_ = -1;
            ...
            if (key != -1) {
                ...
                if (network_interface_ && my_player_id_ > 0) {
                    if (my_player_id_ == 1 && keymap_player1_.find(key) != keymap_player1_.end()) {
                        ...
                        std::cout << "🎯 Player 1 command: " << keymap_player1_[key] << std::endl;
                        ...
                        std::cout << "🎯 Player 2 command: " << keymap_player2_[key] << std::endl;
                        ...
                        std::cout << "❌ Key " << key << " not found in keymap for player " << my_player_id_ << std::endl;
                    }
                }
...
void Game::move_cursor(int player_id, int dx, int dy) {
    std::pair<int, int>& cursor_pos = (player_id == 1) ? cursor_pos_player1_ : cursor_pos_player2_;
    int old_x = cursor_pos.first;
    int old_y = cursor_pos.second;
    ...
    std::cout << "🎯 CURSOR MOVED Player " << player_id << ": (" << old_x << "," << old_y << ") -> (" << cursor_pos.first << "," << cursor_pos.second << ")" << std::endl;
}
